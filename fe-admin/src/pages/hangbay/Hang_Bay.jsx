...
const Hang_Bay = () => {
  ...
  const handleAdd = () => {
    axios.post("http://localhost:8000/api-hang-bay/add", formData)
      .then(() => {
        fetchData();
        setShowForm(false);
        setFormData({
      ...
      {showForm && (
        <div className="bg-gray-50 p-4 border rounded-lg mb-6">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <input name="ma_hang_bay" value={formData.ma_hang_bay} onChange={handleChange} placeholder="Mã hãng bay" className="p-2 border rounded" />
            <input name="ten_hang_bay" value={formData.ten_hang_bay} onChange={handleChange} placeholder="Tên hãng bay" className="p-2 border rounded" />
            <input name="iata_code" value={formData.iata_code} onChange={handleChange} placeholder="IATA Code" className="p-2 border rounded" />
            <input name="quoc_gia" value={formData.quoc_gia} onChange={handleChange} placeholder="Quốc gia" className="p-2 border rounded" />
          </div>
          <div className="mt-4">
      ...
      <div className="overflow-x-auto">
        <table className="min-w-full divide-y divide-gray-200">
          <thead className="bg-gray-100">
            <tr>
              <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Mã hãng bay</th>
              <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Tên hãng bay</th>
